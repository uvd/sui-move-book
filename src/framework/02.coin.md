...
## Sui Move Coin
- 由于ERC20的影响力非常大，后续出现的公链或多或少都受到了以太坊ERC20的影响。Sui Move也是受到了ERC20的影响，但是Move面向资产和所有权的模型，还有很多Sui Move独有的东西，例如object。这使得ERC20在Sui上对应的Coin协议有了很大的创新和差异。总结起来，使用起来更简单且更加通用。
...
## 如何按经济模型锁仓
- 涉及到锁仓的操作一般都会把 Coin 转成 Balance 存储在合约里面

- 涉及动态mint挖矿的一般都会把 TreasuryCap 转成 Supply , 把 Supply 锁定在合约里面 并且做好限制
